...
- **A package has exactly ONE browser half, and dsh serves it through exactly one route.** That route is the combo route for the declared `exports["./client"]` entry (`/plugins/??<pkg>/client.js&rev=…`); no route serves any other file inside the package. A sibling `<script src>` beside the bundle therefore 404s, and the tweak it was meant to install silently never runs — which is what happened to `ui-tweaks`' open-in-editor handler until it was inlined into `client/index.js`, where it now lives in a delimited region. A check that loads two client files into **separate** fake windows cannot catch this: it proves each half alone and never that a page can reach both, so the browser check must drive the factory in ONE sandbox.

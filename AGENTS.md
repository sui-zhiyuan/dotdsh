...
- All build output goes into `target/<language>/`, and `target/` is gitignored: `target/book` (mdbook, active), `target/node` (Node builds — **Node currently has no build step**; when a TS/other build is introduced its outDir must point here), `target/python` (reserved).
...
1. Create a package under `node_src/<id>/` (package.json + src/index.js).
2. Add an entry `{id, package, enabled, config}` to `apps` in `applist.yaml`.
3. `uv run scripts/gen_applist.py` (regenerate the store patch) → verify: sync under a temporary `DSH_HOME` and boot.
